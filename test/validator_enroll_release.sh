...
cmp "$task_dir/release-a/roko-validator-enroll-1.3.0.tar.gz" \
  "$task_dir/release-b/roko-validator-enroll-1.3.0.tar.gz"
cmp "$task_dir/release-a/roko-validator-enroll-1.3.0.metadata.json" \
  "$task_dir/release-b/roko-validator-enroll-1.3.0.metadata.json"
...
(
  ...
  sha256sum roko-validator-enroll-1.3.0.tar.gz \
    roko-validator-enroll-1.3.0.metadata.json \
...
[[ "$("$bin_dir/roko-validator-enroll" --version)" == 'roko-validator-enroll 1.3.0' ]]
"$bin_dir/roko-validator-enroll" --help | grep -F 'Generate a public-only ROKO validator enrollment package' >/dev/null
python3 - "$install_root/1.3.0/compatibility/validator-enroll-compatibility.json" <<'PY'
import json
import sys
...
printf 'tamper' >>"$tampered_archive/roko-validator-enroll-1.3.0.tar.gz"
...
printf ' ' >>"$tampered_metadata/roko-validator-enroll-1.3.0.metadata.json"
