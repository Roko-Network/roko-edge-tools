...
def bounded_rpc_message(value: Any) -> str:
    # RPC error text is untrusted: never echo embedded credentials or paths.
    message = str(value or "").strip().rstrip(".")
    allowed = {
        "RPC call is unsafe to be called externally", "Method not found",
        "Invalid params", "Invalid session keys",
        "Session keys are not encoded correctly", "keystore unavailable",
    }
    return message if message in allowed else "RPC rejection (details redacted)"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check_rpc_policy:
        if args.output or args.check_account or args.expected_session_keys or args.qualification or args.confirm_new_keys or args.session_keys or args.confirm_isolated_unsafe_rpc:
    ...
    if args.check_account:
        ...
        if args.qualification:
            try:
                with open(args.qualification, encoding="utf-8") as handle:
                    qualification = json.load(handle)
            except (OSError, ValueError) as error:
                raise EnrollmentError("Unable to load public qualification JSON") from error
            if not isinstance(qualification, dict):
                raise EnrollmentError("Qualification identity must be an object")
            _, binary_hash = binary_identity(Path(args.binary))
            if qualification.get("binarySha256") != binary_hash:
                raise EnrollmentError("Qualification binary digest does not match the local node binary")
        status = check_transition(RpcClient(args.rpc), args.check_account, args.expected_session_keys, qualification)
        print(json.dumps(status, sort_keys=True, indent=2))
        return 0 if status["safeToEnableValidatorMode"] else 2
    if args.qualification:
        raise EnrollmentError("Qualification identity is only supported for transition checks")
